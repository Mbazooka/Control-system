...
(define max-train-speed-SIM 100)
(define min-train-speed-SIM -100)
(define max-train-speed-HARD 30)
(define min-train-speed-HARD -30)
(define max-train-speed '())
(define min-train-speed '())

;; Constants that determine frame size and title
(define CHOICE-FRAME-WIDTH 300)
(define CHOICE-FRAME-HEIGHT 300)
...
(define (make-gui-adt switch-adjust-cb switch-retrieve-cb
                      ...
                      train-dest-cb train-current-cb train-behind-cb CL-cb)

  (define (process-CL)
    (let ((CL-arg (vector-ref (CL-cb) 0)))
      (cond
        ((string=? CL-arg "hardware") (set! max-train-speed max-train-speed-HARD)
                                      (set! min-train-speed min-train-speed-HARD))
        ((string=? CL-arg "simulator") (set! max-train-speed max-train-speed-SIM)
                                       (set! min-train-speed min-train-speed-SIM)))))
